...
class Plqe(Static):
    ...
    def load_data(self):
        ...
        if self.n_meas != 3 and self.n_meas != 5:
            raise RuntimeWarning(f'Only works with 3 or 5 files, but {self.n_meas} given.')
        ...
        for i,name in enumerate(self.data_list):            
            # extract pump wl
            str_pump = re.search('_\d+nm_', name.lower())
            if str_pump != None:
                laser.append(float(str_pump.group()[1:-3]))
            
            # extract power
            str_power = re.search('_\d+p?\d*?[mu]w_', name.lower())
            if str_power != None:
                power.append(float(str_power.group().replace('p','.')[1:-3]))
            
            # extract center WL (for calibration)
            str_cw = re.search('_(cw)?\d+(cw)?_', name.lower())
            if str_cw != None:
                cw.append(int(re.search('\d+', str_cw.group()).group()))

            # accumulations and exposure
            str_to_parse = re.search('\d+x\d+[p.s0-9]+',name.lower())
        ...
        self.laser_wl = tuple({*laser}) # only save the unique center wavelengths, discard duplicates
        if cw != []:
    ...
    def combine_plqe(self):
        self.combine_wl = min(self.wl[:,1])+50
         #Combine the LAS and PL measurements. Create an intermediate array (inter) with all 5 columns and later remove the PL columns
        inter = self.data_raw[self.wl_raw[:, 0] < self.combine_wl, :] #pick LAS measurement until combineWL
        inter2 = self.data_raw[self.wl_raw[:, 1] >= self.combine_wl,:] # pick PL measurement after combineWL
        interLAS = inter[:, (0,2,4)] # remove columns with with PL experiments
        interPL = inter2[:,(1,3,4)] # remove columns with LAS experiments
        
        #combine WL arrays
        self.wl = np.concatenate(((self.wl[self.wl[:, 0] < self.combine_wl, 0]),
                                  (self.wl[self.wl[:, 1] >= self.combine_wl, 1])))
                                          
        com = np.vstack((interLAS, interPL))
        com [self.wl >= self.combine_wl, -1] = 0 #Set the NO measurement PL region to 0 for a 5 measurement experiment. Can add later a 6 experiment if a NOPL measurement is also performed
        self.data = com
    ...
    def calc_plqe(self,exc_wl=None,pl_wl=None,**kwargs):
        '''Calculates PLQE for the current data integrating over Ex and PL wls as defined by ExWL and PLWL
        author VG last edited 18/05/20'''        
        self.exc_wl = exc_wl
        self.pl_wl = pl_wl
        
        if (self.is_cal == False):
        ...
        pl_on = np.sum(self.data[(self.wl > pl_wl[0]) &
                                 (self.wl < pl_wl[1]),0]) - bgd #Integrate over PL ON
        pl_off = np.sum(self.data[(self.wl > pl_wl[0]) &
                                  (self.wl < pl_wl[1]),1]) - bgd # Integrate over PL OFF
